...
let testId = ""; // test ID (sent back by telemetry if used, null otherwise)

let log = ""; // telemetry log
function tlog(s: string) {
	if (settings.telemetry_level >= 2) {
		log += Date.now() + ": " + s + "\n";
	}
}
function tverb(s: string) {
	if (settings.telemetry_level >= 3) {
		log += Date.now() + ": " + s + "\n";
	}
}
function twarn(s: string) {
	if (settings.telemetry_level >= 2) {
		log += Date.now() + " WARN: " + s + "\n";
	}
	console.warn(s);
...
let xhr: XMLHttpRequest[] | []; // array of currently active xhr requests
let interval: number; // timer used in tests
...
function dlTest(done: () => void) {
	 ...
	 * @param {number} delay
	 */
	const testStream = (i: number, delay: number) => {
		setTimeout(
			() => {
...
function ulTest(done: () => void) {
	...
	const maxInt = 2 ** 32 - 1;
	try {
		const x = new Uint32Array(r);
		for (let i = 0; i < x.byteLength; i++) {
			x[i] = Math.random() * maxInt;
		}
		r = x;
	} catch (e) {}
	const req = [];
			...
			if (i === 0) {
				...
				if (settings.ping_allowPerformanceApi) {
					try {
						...
						const p: PerformanceResourceTiming = pl[pl.length - 1];
				...
				if (i === 1) {
					...
					else jitter = jitter * 0.9375 + instjitter * 0.0625; // update jitter, exponential weighted average
				}
				prevInstspd = instspd;
			}
 ...
 */
function sendTelemetry(done: (id?: string) => void) {
