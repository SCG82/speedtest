...
class Speedtest {
    ...
    private _state: 0|1|2|3|4;
    onupdate: Function | undefined;
    onend: Function | undefined;
    worker: Worker | undefined;
    private _prevData: any;
    updater: number | undefined;
    private _originalExtra: any;
    private _selectServerCalled: any;
    constructor() {
        this._serverList = []; //when using multiple points of test, this is a list of test points
        this._selectedServer = undefined; //when using multiple points of test, this is the selected server
        // this._settings = undefined; //settings for the speedtest worker
     ...
     * @param url - the url where the server list can be fetched.
     ...
     * getIpURL: "getIP.php", // path to getIP.php or its replacement on the server
     * ```
     * @param result - callback to be called when the list is loaded correctly.
     ...
     */
    selectServer(result: (arg0?: Server) => void) {
        if (this._state !== 1) {
         ...
         * @param {(arg0: Server) => void} result
         */
        const select = (serverList: Server[], result: (arg0: Server | null) => void) => {
             ...
             * @param {string} url
             * @param {(pingMs: number) => void} result
             */
            const ping = (url: string, result: (pingMs: number) => void) => {
                ...
                xhr.onload = () => {
                    ...
                    if (xhr.responseText.length === 0) {
                        ...
                        try {
                            ...
                            const p: PerformanceResourceTiming = pArr[pArr.length - 1];
             ...
             * @param {() => void} done
             */
            const checkServer = (server: Server, done: () => void) => {
                let i = 0;
                server.pingT = -1;
